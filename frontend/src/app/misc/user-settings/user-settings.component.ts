...
export class UserSettingsComponent implements OnInit {
    ...
    changeUser() {
        this.dialogService.confirm('Sind Sie sich sicher, Ihr Profil zu ändern?', 'Profil Ändern').afterClosed().subscribe(confirm => {
            if(confirm) {
                this.authService.changeUser(new User(this.username, this.email, this.user.id, false, this.name, this.regionalgruppe as any)).subscribe(res => {
                    if (res.successful) {
    ...
    changePassword() {
        this.dialogService.confirm('Sind Sie sich sicher, Ihr Passwort zu ändern?', 'Passwort Ändern').afterClosed().subscribe(confirm => {
            if(confirm) {
                this.authService.changePassword(this.user, this.password).subscribe(res => {
                    if (res.successful) {
                        this.dialogService.flashSuccess('Der Passwort wurde erfolgreich verändert.');
                    } else {
